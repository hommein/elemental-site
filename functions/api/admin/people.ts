...
export const onRequestGet: PagesFunction<AuthEnv> = async ({ env, request }) => {
  ...
  const su = (await env.DB.prepare(
    `SELECT s.email, s.date, c.title, c.time FROM signups s JOIN classes c ON c.id=s.class_id
  ...
  for (const s of su) guest(s.email.toLowerCase()).classes.push({ date: s.date, title: s.title, time: s.time });
