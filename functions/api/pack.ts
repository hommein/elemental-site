...
export const onRequestGet: PagesFunction<AuthEnv> = async ({ env, request }) => {
  ...
  const pays = (await env.DB.prepare("SELECT date,amount,method FROM payments WHERE user_id=?1 ORDER BY id DESC LIMIT 5").bind(u.id).all()).results;
  return json({ packs, payments: pays });
};
